...
def update_dataframe_texts(df, common_words_by_pair, label_column='class_label', text_column='text'):
    ...
    FIXED: Split on ' - ' with maxsplit=1 to handle class names containing hyphens.
    """
    for key, common_words in common_words_by_pair.items():
        # Split on ' - ' (with spaces) and only split once
        parts = key.split(' - ', 1)
        
        # Handle malformed keys
        if len(parts) != 2:
            print(f"Warning: Skipping malformed key '{key}' - expected format 'class1 - class2'")
            continue
        
        class1, class2 = [s.strip() for s in parts]
        
        # Convert common words to lowercase for case-insensitive matching
        lower_common_words = set(word.lower() for word in common_words)
        
        # Create mask for rows matching class2
        mask = df[label_column].str.strip() == class2
        
        # Apply synonym replacement to matching rows
