...
- Tests DIRECTLY after training (no model reloading)
"""
...
class SimpleMedicalEvaluator:
    """Evaluator that works with the model in memory (no loading required)"""
    
    def __init__(self, model, tokenizer, text_processor):
        # Takes the trained model directly from memory
        self.model = model
        self.tokenizer = tokenizer
    ...
    def predict(self, clinical_text: str, max_new_tokens: int = 50) -> Tuple[str, Dict]:
        """Make prediction using the in-memory model"""
    ...
    def evaluate(self, test_file: str, max_samples: int = None) -> Dict:
        ...
        print("Starting evaluation on TRAINED MODEL (in memory, not reloaded)...")
        ...
        for i, item in enumerate(test_data, 1):
            pred_response, _ = self.predict(item['clinical_text'], max_new_tokens=50)
...
class GenerativeMedicalFineTuner:
    ...
    def fine_tune(self, train_dataset, val_dataset):
        ...
        out_dir = self.config.get('output_dir', './generative_medical_loracssfull')
    ...
    def evaluate_on_test_set(self, test_file: str):
        """Evaluate using the TRAINED model directly from memory (NO reloading)"""
        print("\n" + "="*80)
        print("TESTING ON TRAINED MODEL (DIRECTLY FROM MEMORY - NO RELOADING)")
        print("="*80)
        ...
        # Pass the trained model directly to evaluator
        print("Creating evaluator with TRAINED model from memory...")
        evaluator = SimpleMedicalEvaluator(self.model, self.tokenizer, self.text_processor)
        
        print("Starting evaluation on FULL test dataset...")
        results = evaluator.evaluate(test_file, max_samples=None)
        
        results_file = os.path.join(self.config.get('output_dir', './generative_medical_loracssfull'), 
        ...
        with open(results_file, 'w') as f:
            json.dump({
                'evaluation_type': 'test_set_evaluation_immediately_after_training',
                'test_file': test_file,
                'timestamp': pd.Timestamp.now().isoformat(),
                'model_status': 'trained_in_memory_not_reloaded',
    ...
    def run_full_pipeline(self, train_file: str, val_file: str, test_file: str = None) -> bool:
        print("GENERATIVE MEDICAL DIAGNOSIS TRAINING + IMMEDIATE TESTING")
        print("=" * 60)
        ...
        if test_file:
            print("\n" + "="*80)
            print("RUNNING TEST SET EVALUATION ON TRAINED MODEL")
            print("="*80)
...
def main_generative_finetune():
    config = {
        ...
        "learning_rate": 5e-5,
        "weight_decay": 0.01,
        "output_dir": "./generative_medical_loracssfull",
        "logging_steps": 10,
        "eval_steps": 500,
        "save_steps": 1000,
        "seed": 42,
    ...
    print("TRAINING + IMMEDIATE TEST EVALUATION (NO MODEL RELOADING)")
    print("="*60)
    ...
    try:
        ...
        if success:
            print("\nTraining and IMMEDIATE test evaluation completed!")
